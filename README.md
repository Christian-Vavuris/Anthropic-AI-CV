# Christian Vavuris — Anthropic Candidate Site

A single public page: name, LinkedIn, one-line positioning, and a chat panel
that answers questions about Christian's fit for Anthropic's Account
Executive role (AI Native / Enterprise, Tech) — backed by Claude and the
same knowledge base as the `christian-vavuris` skill.
...
and the `christian-vavuris` Claude skill. When one is updated, copy the
change to the other manually.
...
This is a standalone project — separate git history, separate `node_modules`,
separate Vercel deployment from the Stripe candidate site in `../Job Sites`
and the Middesk candidate site in `../Middesk Job Site`.
